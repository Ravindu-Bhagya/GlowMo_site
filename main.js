...
const _adminBtn = document.getElementById('adminBtn');
if (_adminBtn) _adminBtn.onclick = handleAdminClick;
updateAdminUI();
...
let products = [
  { id: genId(), name: 'APLB Glutathione Niacinamide Ampoule Serum', cat: 'Face', price: 8500, stock: 10, img: 'images/APLB Glutathione Niacinamide Ampoule Serum.jpeg' },
  { id: genId(), name: 'Cetaphil Gentle Skin Cleanser For All Skin Types', cat: 'Body', price: 9900, stock: 5, img: 'images/Cetaphil Gentle Skin Cleanser For All Skin Types.jpeg' },
  { id: genId(), name: 'Dr.G Black Snail Cream', cat: 'Face', price: 10500, stock: 5, img: 'images/Dr.G Black Snail Cream.jpeg' },
  { id: genId(), name: 'Glutathione + Niaciamide sheet mask.jpeg', cat: 'Face', price: 1390, stock: 8, img: 'images/Glutathione + Niaciamide sheet mask.jpeg' }
];
...
function renderCart() {
  ...
  keys.forEach(id => {
    const it = cart[id]; totalQ += it.qty; totalP += Number(it.price) * it.qty;
...
function attachCartEvents() {
  document.querySelectorAll('.inc').forEach(b => b.onclick = () => {
    const id = b.dataset.id; const p = products.find(x => x.id === id); if (p.stock <= 0) { alert('No more stock'); return; }
    cart[id].qty++; p.stock--; saveProducts(); renderCart(); renderProducts();
  });
  document.querySelectorAll('.dec').forEach(b => b.onclick = () => {
    const id = b.dataset.id; if (cart[id].qty > 1) { cart[id].qty--; const p = products.find(x => x.id === id); p.stock++; saveProducts(); renderCart(); renderProducts(); }
  });
  document.querySelectorAll('.removeItem').forEach(b => b.onclick = () => {
    const id = b.dataset.id; const p = products.find(x => x.id === id); p.stock += cart[id].qty; delete cart[id]; saveProducts(); renderCart(); renderProducts();
  });
}
  ...
  return new Promise((resolve) => {
    const reader = new FileReader(); reader.onload = function (ev) {
      const img = new Image(); img.onload = function () {
        const canvas = document.createElement('canvas'); const ratio = img.width / img.height; let w = img.width; let h = img.height;
